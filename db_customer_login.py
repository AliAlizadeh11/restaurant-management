...
def extract_national_code(email_or_national_code):
    ...
    c.execute(f"SELECT * FROM customer WHERE email == '{email_or_national_code}' or national_code == '{email_or_national_code}'")
    items = c.fetchone()
    return items
...
def query_login(email, password):
        """query to database"""
        c.execute(f"SELECT * FROM customer WHERE email = '{email}'")
        items = c.fetchone()
